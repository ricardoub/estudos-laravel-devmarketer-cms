...
<template>
  ...
  <div class="slug-widget">
    ...
    <div class="url-wrapper wrapper">
      ...
      <input type="text" name="slug" id="slug-editor" class="input is-small" 
        v-show="isEditing" v-model="customSlug" @keyup="adjustWidth" />
    </div>

    <div class="button-wrapper wrapper">
      <button class="save-slug-button button is-small" v-show="!isEditing" @click.prevent="editSlug">
        {{slug.length < 1 ? 'Create New Slug' : 'Edit'}}
      </button>
      <button class="save-slug-button button is-small" v-show="isEditing" @click.prevent="saveSlug">
        {{customSlug == slug ? 'Cancel' : 'Save'}}
      </button>
  ...
  export default {
    ...
    methods: {
      adjustWidth: function(event) {
        ...
        const slugEditorInput = document.getElementById('slu-editor');
      ...
      setSlug: function(newVal, count = 0) {
        if (newVal === '') return '';
        // Slugify the newVal with the Slug component
